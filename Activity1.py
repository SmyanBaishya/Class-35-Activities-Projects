...
# Custom event ID for colour change events
...
class Sprite(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        boundary_hit = False
        # Check for collision with top or bottom boundaries and reverse directions
...
while not exit:
    ...
    for event in pygame.event.get():
        ...
        # If the sprite color change event is triggered, change the sprite's color
            ...
            sp1.change_color()
        # If the background color change event is triggered, change the sprite's color
        elif event.type == BACKGROUND_COLOR_CHANGE_EVENT:
            change_background_color()

    # Update all sprites
    all_sprites_list.update()
    # Fill the screen with the correct background color
    screen.fill(bg_color)
